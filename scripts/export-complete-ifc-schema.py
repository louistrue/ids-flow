...
def export_property_sets():
    """Export property sets from bSDD API"""
    ...
    for version in versions:
        print(f"\n📋 Exporting {version} property sets...")
        
        try:
            # Try bSDD API first
            url = f"https://identifier.buildingsmart.org/uri/ifc/{version.lower()}/property-sets"
            response = requests.get(url, timeout=10)
            
            if response.status_code == 200:
                data = response.json()
                property_sets = data.get('propertySets', [])
                print(f"  ✅ Fetched {len(property_sets)} property sets from bSDD")
            else:
                print(f"  ⚠️  bSDD API failed ({response.status_code}), using comprehensive fallback")
                property_sets = generate_comprehensive_property_sets(version)
            ...
            with open(output_file, 'w') as f:
                json.dump(property_sets, f, indent=2)
            
            print(f"  📁 Saved to {output_file}")
            
        except Exception as e:
            print(f"  ❌ Error exporting {version} property sets: {e}")
            # Use fallback
            property_sets = generate_comprehensive_property_sets(version)
